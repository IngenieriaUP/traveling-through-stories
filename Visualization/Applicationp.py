...
if st.checkbox('Análisis general'):
	#caja1,caja2=st.beta_columns(2)
	macrodown=['Selecciona una opción','Mapa',"Mostrar patrones frecuentes","Extraer patrones frecuentes"]
	choice = st.selectbox("Menu",macrodown)
	if choice=="Mapa":
		...
		st.plotly_chart(fig)
	#caja2.header("Patrones frecuentes")
	if choice=="Mostrar patrones frecuentes":
		muestra=st.text_input('Número de patrones a mostrar')
		if muestra!='':
			df2=pd.read_csv('pospattern.csv',sep=';')
			st.write(df2.head(int(muestra)))
	#caja1.markdown("Extraer los patrones del texto")
	if choice=="Extraer patrones frecuentes":
		st.write('Espere mientras tanto')
		spatronesintax(df)
	else:
		st.write(' ')
#Check micro
if st.checkbox('Análisis específico'):
	cap=listar(df)
	...
	def dep_analyzer(my_text):
		...
		return png
	#seccion entidades
	microd=["Selecciona una opción","Reconocimiento de entidades","Árbol de sintaxis"]
	choice2 = st.selectbox("Menu",microd)
	#st.header("Reconocimiento de entidades")
		#mostrar las entidades en el texto
	if choice2=="Reconocimiento de entidades":
	...
	if choice2=="Árbol de sintaxis":
		html=dep_analyzer(my_text)
		st.image(html,format='JPEG')
	else:
		st.write(' ')
