...
st.sidebar.markdown('Esta app es una herramienta para el análisis de textos del género literario relatos de viajes. Se realiza un análisis macro y micro del texto. En el análisis macro se tiene dos modulos, un mapa el cual muestra los personajes relacionados con la locaciones y un extractor de patrones de sintaxix que muestra los patrones más frecuentes del texto. Por otra parte, en el análisis micro se presenta una selección por capítulo, parrafo y oración del texo el cual aparecerá en el cuadro de texto. Asimismo, dentro del análisis se reconoce las entidaes y genera un árbol de sintaxis.')
#Check Macro
if st.checkbox('Análisis macro'):
	if st.checkbox("Mapa"):
		...
		st.plotly_chart(fig)
	if st.checkbox("Patrones frecuentes"):
		st.markdown("Mostrar los patrones extraidos")
...
if st.checkbox('Análisis micro'):
	...
	if st.checkbox("Árbol de sintaxis"):
